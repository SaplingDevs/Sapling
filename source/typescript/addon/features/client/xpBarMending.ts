...
import type { Container, ItemStack, Player } from "@minecraft/server";

export default function xpBarMending() {
  const parsedPlayers = world.getPlayers({ tags: [ 'client:xpBarMending' ] });
  parsedPlayers.forEach((player) => xpBarMendingFeature(player))
}

function xpBarMendingFeature(player: Player) {
  if (!player.isSneaking) return;
  // Data
  const inventory = player.getComponent('inventory').container;
  const item = inventory.getItem(player.selectedSlotIndex);

  if (!item || !item.hasComponent(ItemComponentTypes.Durability) || !item.hasComponent(ItemComponentTypes.Enchantable)) return;

  const durability = item.getComponent(ItemComponentTypes.Durability);
  const hasMending = item.getComponent(ItemComponentTypes.Enchantable).hasEnchantment("mending");
...
function replaceItem(player: Player, item: ItemStack, newDurability: number, inventory: Container) {
  const itemData = {
